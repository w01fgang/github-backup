...
const SCENARIOS: Scenario[] = [
  ...
  },
  {
    ...
    mode: "manual",
    manualInstruction:
      "Operator: run `npm run verify:phase-4 -- --inject-ref-mismatch`. The flag writes `refs/heads/__verify_mismatch__` into the restored bare mirror at the only moment a divergence can exist — after verify's restore has cloned from the droplet, before it compares — so the real Group 2 detector, diagnostic and exit path are exercised. Hand-injecting around the verifier instead does not work: restore clones FROM the droplet, so a ref added first is copied into both mirrors and the run exits 0 having asserted nothing. Verify: exit 1, `✗ ref mismatch between droplet mirror and restored bare mirror`, `local-only count : 1`, and the `__verify_mismatch__` line under `first 1 local-only`. Exit 2 means the detector failed to fire; exit 0 is impossible here. Then run plain `npm run verify:phase-4` and verify exit 0 — the injection lives only in verify's own temp mirror, so the droplet needs no cleanup. Per plan 10-01 the D-02 floor pins this scenario to `manual`; the runner does not automate it even though the flag never writes to the droplet. Temp dirs are left on disk for inspection (D-06).",
  },
