...
1. ~~Verify + fix the two Codex-only HIGH security items (webhook body cap; deny-list on the registration/webhook path).~~ **Done** — both confirmed real and fixed in `8f83336` (PR #5). Codex's HIGH was the right call: the deny-list bypass rested on Phase 9's "a per-repo webhook is explicit consent" rationale, which `register-webhooks.ts` invalidates by auto-registering hooks on every admin-capable repo. A third bypass surfaced while fixing it — `filter-repos.sh` pathname-expanded its own glob patterns, making deny lists depend on the caller's working directory (`deny: ["*"]` denied nothing from a non-empty cwd). That one also affected the cron path.
...
- **Canonical verification evidence.** `10-02-PLAN.md` task 02-03 step 5: for a phase whose HUMAN-UAT reached `pending: 0` with no `blocking:` failure, `03-VERIFICATION.md` / `04-VERIFICATION.md` flip `status: human_needed` → `status: passed` (04 also retires its `code-shipped-pending-live-smoke` requirement statuses) and gain the Phase 10 closure-note pointer; task 02-04 refuses to mark a `verification_gap` STATE row `resolved` while its canonical file still reads `human_needed`. The plan's earlier blanket `DO NOT touch` on those two files cited D-03, which says no such thing (`10-CONTEXT.md:72-77`) — D-03 in fact expects both `verification_gap` rows to close in Phase 10 (`:75`), and 02-03 already back-edits HUMAN-UAT files in the same v1.0 archive.
...
**Both Codex-only HIGH items shipped separately** in `8f83336` (PR #5), after this review branch opened: the listener caps the request body at `WEBHOOK_MAX_BODY_BYTES` before the HMAC check (413), and `webhook-listener.js` + `register-webhooks.ts` both enforce the source's allow/deny globs through `droplet/lib/filter-repos.sh` (403 `repo_denied`; no hook on a denied repo). WEBHOOK-04 is reinstated in `REQUIREMENTS.md`. Still unverified against a live droplet — `verify:phase-3` assertions for the new 403/413 responses are a follow-up.
