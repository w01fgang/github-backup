...
| `D-08` | A slug resolves to its mirror through `scripts/lib/mirror-path.ts`: the droplet lists every mirror it holds and the match is made locally, case-insensitively, with an exact-case hit taking precedence. Mirror dirs carry GitHub's canonical `full_name` casing while slugs are case-insensitive, so a case-sensitive shell glob reported "no mirror" for correctly-spelled slugs. | `scripts/lib/mirror-path.ts`, `scripts/restore.ts`, `scripts/verify/phase-4.ts` |
