...
| `D-08` | A slug resolves to its mirror through `scripts/lib/mirror-path.ts`: the narrowing runs on the droplet, as one `find -L <backupDir> -mindepth 2 -maxdepth 2 -type d -iname '<owner>_<repo>.git'`, so the reply is bounded by how many mirrors match the slug and never by how many the droplet holds — listing the whole inventory back and matching locally puts a large enough backup set over `execSync`'s 1 MiB output buffer and fails every restore with `ENOBUFS`. Mirror dirs carry GitHub's canonical `full_name` casing while slugs are case-insensitive, so the case-sensitive shell glob this replaced reported "no mirror" for correctly-spelled slugs; `-iname` supplies the case-insensitive match remotely. `-L` keeps the glob's symlink transparency, which `find`'s default `-P` drops: both a `backupDir` on a symlinked volume and an individually symlinked mirror stay findable, the latter because `sync-one-repo.sh`'s `[[ -d ]]` follows the link and keeps updating it. A dangling symlink stays unmatched, so the caller bails instead of handing `git clone` an unreadable path. Every case-variant hit is returned, never narrowed to an exact-case one: variants are the same repo mirrored twice with one copy stale, so the caller's ambiguity bail decides. | `scripts/lib/mirror-path.ts`, `scripts/restore.ts`, `scripts/verify/phase-4.ts` |
