...
| `D-08` | A slug resolves to its mirror through `scripts/lib/mirror-path.ts`: the droplet lists every mirror it holds and the match is made locally, case-insensitively. Mirror dirs carry GitHub's canonical `full_name` casing while slugs are case-insensitive, so a case-sensitive shell glob reported "no mirror" for correctly-spelled slugs. Every case-variant hit is returned, never narrowed to an exact-case one: variants are the same repo mirrored twice with one copy stale, so the caller's ambiguity bail decides. | `scripts/lib/mirror-path.ts`, `scripts/restore.ts`, `scripts/verify/phase-4.ts` |
